...
hide_window() {
    ...
    ensure_floating "$id"
    aerospace move-node-to-workspace "$SCRATCHPAD" --window-id "$id"
    # Trigger sketchybar update to hide scratchpad badge
    sketchybar --trigger aerospace_workspace_change FOCUSED_WORKSPACE="$CURRENT_WS" 2>/dev/null || true
}
...
show_window() {
    ...
    ensure_floating "$id"
    aerospace focus --window-id "$id"
    # Trigger sketchybar update to show scratchpad badge
    sketchybar --trigger aerospace_workspace_change FOCUSED_WORKSPACE="$CURRENT_WS" 2>/dev/null || true
}
